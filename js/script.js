...
$('.sup_form').submit(function() { 
	...
	function products(){
		var current = 0,
	    order = $('#header_slider .buy');
	    order_amount = order.length;
	    $('#header_slider').fadeIn(400);
	    $(order.css('position', 'absolute').hide().get($('.buy:hidden').eq(Math.floor(Math.random() * $('.buy:hidden').length))-1)).show();

	    window.setInterval(select_order, 10500);
	...
	$(".screen1 nav li a").click(function() {
